...
arquivo = 'DATAGRAMA/imgrecebida.png'
...
def datagrama(img):
    ...
    for payload in split_txBuffer:
        head = (len(payload).to_bytes(3,byteorder='big') + n_pacote.to_bytes(3,byteorder='big') + total_pacotes.to_bytes(3,byteorder='big'))
        ...
        print('lista')
    print('LISTA DOS PACOTES A SEREM ENVIADOS: ', lista_pacotes_a_serem_enviados)
...
def main():
    try:
        ...
        head = (nRx.to_bytes(3,byteorder='big')+ n_pacote.to_bytes(3,byteorder='big') + total_pacotes.to_bytes(3,byteorder='big'))
        while len(head) < 10:
            head += b'\x00'
